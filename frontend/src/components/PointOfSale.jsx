  ...
  const handleCheckout = async () => {
    ...
    try {
      const saleData = {
        ...
        tax_amount: calculateTax(calculateSubtotal())
      };

      await axios.post(`${API_BASE}/sales`, saleData);
      alert('Sale completed successfully!');
      setCart([]); // Clear cart after sale
      // Optionally, refetch products to update stock (implement backend to deduct stock)
      const updatedProducts = await fetchProducts();
      setProducts(updatedProducts);
    } catch (err) {
      console.error('Error processing sale:', err.response?.data || err.message);
